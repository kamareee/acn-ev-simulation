...
class AdaptiveEventQueue(BaseSimObj):
    ...
    def add_event(self, event):
        ...
        Returns:
            None
        """
        # Changed here. If the event is a tuple, it is a (event, priority) pair.
        if issubclass(type(event), tuple):
            heapq.heappush(self._queue, (event[0].timestamp, event[0], event[1]))
        else:
    ...
    def get_event(self):
        ...
        """
        # Changed here to handle special event (event, priority) pair.
