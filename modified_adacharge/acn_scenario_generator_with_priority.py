...
class ScenarioResult:
    ...
    def get_session_summary(self) -> Dict:
        ...
        return {
            ...
            "priority_pct": (
                self.n_priority / self.n_sessions * 100 if self.n_sessions > 0 else 0
            ),
            ...
            "priority_avg_energy_kwh": (
                np.mean(priority_energies) if priority_energies else 0
            ),
        }
...
class ACNScenarioGenerator:
    ...
    def generate_scenario(
        self, scenario_key: str, n_sessions: int, seed: Optional[int] = None
    ) -> ScenarioResult:
        """
        ...
        sessions = self._generate_session_params(
            n_sessions=n_sessions, arrival_pattern=config["arrival_pattern"]
        )

        # Select priority sessions
        priority_ids = self._select_priorities(
            sessions=sessions, target_priority_pct=config["priority_pct"]
        )
    ...
    def _sample_arrival_hour(
        self, pattern: str, start_hour: float, end_hour: float
    ) -> float:
    ...
    def _select_priorities(
        self, sessions: List[SessionParams], target_priority_pct: float
    ) -> Set[str]:
        ...
        high_energy_count = 0
        max_high_energy = max(
            1, int(n_target * config.max_high_energy_pct / config.max_priority_pct)
        )
    ...
    def _create_event_queue(self, sessions: List[SessionParams]) -> "EventQueue":
        """
